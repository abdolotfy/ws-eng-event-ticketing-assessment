...
import { bookingsAPI, getWaitlistPosition, leaveWaitlist } from "@/lib/api";
...
export default function BookingsPage() {
  ...
  const [isLoadingWaitlistPositions, setIsLoadingWaitlistPositions] = useState(false);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setIsLoadingWaitlistPositions(true);

    Promise.allSettled(
      waitlistedBookings.map(async (booking) => {
        const response = await getWaitlistPosition(token, booking.eventId);
        return {
      ...
      .then((results) => {
        ...
        for (const result of results) {
          if (result.status === "fulfilled") {
            nextPositions[result.value.bookingId] = result.value.position;
          }
        }
        setWaitlistPositions(nextPositions);
      })
      .finally(() => {
        if (!cancelled) {
          setIsLoadingWaitlistPositions(false);
        }
      });
  ...
  const refreshBookings = async () => {
    if (!token) return;
    const response = await bookingsAPI.list(token);
  ...
  const handleLeaveWaitlist = async (booking: Booking) => {
    if (!token) return;

    setLeavingWaitlistId(booking.id);
    setError("");
    setCancelSuccess("");

    try {
      await leaveWaitlist(token, booking.eventId);
      ...
      {bookings.length === 0 ? (
        ...
        <div className="space-y-4">
          {bookings.map((booking) => {
            const waitlistPosition = waitlistPositions[booking.id];
              ...
              <Card key={booking.id}>
                <CardContent className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {booking.seatTier && (
                      <p className="text-sm text-sky-600 font-medium">
                        ...
                        {booking.status === "WAITLISTED" ? "Waitlist" : formatCurrency(booking.pricePaid)}
                      </p>
                    )}
                    {!booking.seatTier && booking.pricePaid > 0 && booking.status !== "WAITLISTED" && (
                      ...
                      <div className="mt-2 flex items-center gap-2 flex-wrap">
                        <Badge variant="warning">
                          {typeof waitlistPosition === "number"
                            ...
                            : isLoadingWaitlistPositions
                              ? "Loading waitlist position..."
                              : "Waitlist Position unavailable"}
                        </Badge>
                      </div>
                    )}
                    <p className="text-xs text-gray-400 mt-1">
                      Ticket: {booking.ticketCode.slice(0, 8).toUpperCase()}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {booking.status === "WAITLISTED" && (
                      <>
                        <Badge variant="warning" className="px-4 py-2 justify-center">
                          {typeof waitlistPosition === "number"
                            ? `Waitlist Position #${waitlistPosition}`
                            : "Waitlisted"}
                        </Badge>
                        <Button
                          size="sm"
                          variant="secondary"
                          onClick={() => handleLeaveWaitlist(booking)}
                          isLoading={leavingWaitlistId === booking.id}
                        >
                          Leave Waitlist
                        </Button>
                      </>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-900 font-medium">
                    {formatCurrency(refundPreview.pricePaid)}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Refund ({refundPreview.refundPercentage}%)</span>
                  <span className="text-gray-900">
                    {formatCurrency(refundPreview.refundBeforeFee)}
                  </span>
                </div>
